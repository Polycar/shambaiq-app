...
interface HistoryRecord extends DiagnosisResult {
  id: string;
  county: string | null;
  crop: string | null;
  ...
  useEffect(() => {
    async function loadHistory() {
      try {
        ...
        // silently skip if history can't load
      } finally {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResult(data);
        // Prepend to local history so it appears immediately without a reload
        setHistory((prev) => [
          {
            ...data,
        ...
        setError(`AI Error: ${errData.error || res.status}`);
      }
    } catch {
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-cream-300">
              ...
              {!image ? (
                ...
                <div className="space-y-3">
                  ...
                  >
                    {loading ? <><Loader2 size={18} className="animate-spin" /> Analyzing...</> : <>🔍 Analyze with AI</>}
                  </button>
                </div>
              )}
            </div>
            ...
            {result && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border-l-4 border-l-red-500 border border-cream-300">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="font-display text-lg font-bold text-forest-700">{result.condition}</h3>
                    {result.confidence > 0 && (
                      <span className="text-xs text-soil-400">Confidence: {result.confidence.toFixed(1)}%</span>
                    )}
                  </div>
                </div>
                <div className="space-y-4 text-sm">
                  <div>
                    <h4 className="font-semibold text-forest-700 mb-1">🩺 Treatment</h4>
                    <p className="text-soil-500 leading-relaxed">{result.treatment}</p>
                  </div>
                  <div>
                    <h4 className="font-semibold text-forest-700 mb-1">🛡️ Prevention</h4>
