...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { county, acres, soil, lat, lon } = body;
...
Analyze the following soil properties and farm specifications:
- Location: ${county} County, Kenya ${lat && lon ? `(Coordinates: ${lat}, ${lon})` : ''}
- Farm Size: ${farmSize} acres
${soilContext}

Recommend the top 5 absolute best-suited crops that would thrive in this specific soil chemistry and texture. 
You are highly encouraged to suggest any highly profitable, viable Kenyan crops, including those outside standard staple databases (e.g., Hass Avocado, Tea, Coffee Robusta/Arabica, Macadamia, Pixie Oranges, Pyrethrum, Garlic, Arrowroot, Sunflower, Lucerne, Watermelon, etc.), provided they fit the soil's pH, texture, organic carbon, and NPK profile perfectly.

For each crop, you must calculate:
1. A realistic "match_score" (an integer from 0 to 100) representing how well the crop fits the soil parameters.
...
4. A highly specific 1-2 sentence explanation in English ("reason") explaining why this crop fits these precise chemical (pH, NPK, OC) and physical (Texture) soil metrics.
